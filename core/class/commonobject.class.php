...
// creacion de printobjectline para mostrar nota de pedido
function printObjectLinespedido($action, $seller, $buyer, $selected=0, $dateSelector=0)//machfree
	{
		global $conf,$langs,$user,$object,$hookmanager;
        ...
        foreach ($this->lines as $line)
        {
            ...
            {
                $this->printObjectLinepedido($action,$line,$var,$num,$i,$dateSelector,$seller,$buyer,$selected,$extrafieldsline);
            }
	 ...
	 *	@return	void
	 */
	function printObjectLinepedido($action,$line,$var,$num,$i,$dateSelector,$seller,$buyer,$selected=0,$extrafieldsline=0)//machfree
	{
		global $conf,$langs,$user,$object,$hookmanager;
		global $form,$bc,$bcdd;
		global $object_rights, $disableedit, $disablemove, $disableremove;   // TODO We should not use global var for this !
		...
		if ($action != 'editline' || $selected != $line->id)
		{
			...
			foreach($dirtpls as $reldir)
			{
				$tpl = dol_buildpath($reldir.'/objectline_view.pedido.tpl.php');
				if (empty($conf->file->strict_mode)) {
					$res=@include $tpl;
				} else {
					$res=include $tpl; // for debug
		...
		if ($this->statut == 0 && $action == 'editline' && $selected == $line->id)
		{
			...
			foreach($dirtpls as $reldir)
			{
				$tpl = dol_buildpath($reldir.'/objectline_edit.pedido.tpl.php');
				if (empty($conf->file->strict_mode)) {
					$res=@include $tpl;
				} else {
					$res=include $tpl; // for debug
