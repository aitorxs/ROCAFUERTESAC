...
if ($type == 'c') {
    ...
    if ($search_type == '') $search_type = '1,3';
}
if ($type == 'p') {
    $contextpage = 'prospectlist';
    if ($search_type == '') $search_type = '2,3';
}
if ($type == 'f') {
    $contextpage = 'supplierlist';
    if ($search_type == '') $search_type = '4';
}
...
if (empty($reshook)) {
    ...
    if ($action == 'setstcomm') {
        ...
        if ($result < 0) setEventMessages($object->errors, $object->errors, 'errors');
...
if (version_compare(DOL_VERSION, 3.8) >= 0) {
    if (!empty($search_categ)) $sql .= ' LEFT JOIN ' . MAIN_DB_PREFIX . "categorie_" . ($type == 'f' ? "fournisseur" : "societe") . " as cs ON s.rowid = cs.fk_societe"; // We'll need this table joined to the select in order to filter by categ
} else {
...
llxHeader('', $langs->trans("ImpressProducts"), $help_url);
        ...
        if ($user->rights->societe->lire) {
            ...
            if ($num) {
                if ($conf->global->LABELPRINT_THIRD_A4) {
                    ...
                    print $form->formconfirm($_SERVER["PHP_SELF"], $langs->trans('SelectPosition'), $langs->trans('ConfirmSelectPosition'), 'confirm_position', $formquestionposition, 'yes', 'action-position', 170, 400);
                } else {
                    print '<a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?action=confirm_position&amp;confirm=yes">' . $langs->trans('PrintLabels') . '</a>';
                }
            }
        }
    } else {
...
// Action column
print '<td class="liste_titre" align="right"><input class="liste_titre" name="button_search" type="image" src="' . img_picto($langs->trans("Search"), 'search.png', '', '', 1) . '" value="' . dol_escape_htmltag($langs->trans("Search")) . '" title="' . dol_escape_htmltag($langs->trans("Search")) . '">';
...
while ($i < min($num, $limit)) {
    ...
    // Action column
    print '<td align="center">';
    print '<input id="' . $i . '" class="flat checkforproduct" type="checkbox" name="toPrint[]" value="' . $obj->rowid . '">';
