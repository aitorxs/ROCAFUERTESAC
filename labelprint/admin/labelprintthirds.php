...
if (GETPOST("save")) {
    ...
    $res += dolibarr_set_const($db, 'LAB_THIRD_COMP', trim(GETPOST("labComp")), 'chaine', 0, '', $conf->entity);
    $res += dolibarr_set_const($db, 'LAB_THIRD_ADDRESS', trim(GETPOST("labAddress")), 'chaine', 0, '', $conf->entity);
    $res += dolibarr_set_const($db, 'LAB_THIRD_FREE_TEXT', trim(GETPOST("labFreeText")), 'chaine', 0, '', $conf->entity);
    $res += dolibarr_set_const($db, 'LAB_START', 0, 'chaine', 0, '', $conf->entity);
    $res += dolibarr_set_const($db, 'LABELPRINT_THIRD_A4', GETPOST("labA4"), 'chaine', 0, '', $conf->entity);
...
if ($action == 'setlabel') {
    ...
    $labelid = "MAIN_MODULE_LABELPRINT_THIRD_" . $value;
...
if ($action == 'settypelabel') {
    ...
    $labelid = "MAIN_MODULE_LABELPRINT_LABELS_THIRD_" . $value;
...
// read const
$labcomp = dolibarr_get_const($db, "LAB_THIRD_COMP", $conf->entity);
$labaddress = dolibarr_get_const($db, "LAB_THIRD_ADDRESS", $conf->entity);
...
dol_fiche_head($head, 'configthirds', $langs->trans("Labels"), 0, 'barcode');
...
$profid[3][0] = $langs->trans("ThirdParties");
$profid[3][1] = $langs->trans('MenuThirdCardDesc');
$profid[4][0] = $langs->trans("MenuThirds");
$profid[4][1] = $langs->trans('MenuThirdsDesc');
...
while ($i < $nbofloop) {
    ...
    switch ($i) {
        case 0:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_THIRD_0 ? false : true);
            break;
        case 1:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_THIRD_1 ? false : true);
            break;
        case 2:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_THIRD_2 ? false : true);
            break;
        case 3:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_THIRD_3 ? false : true);
            break;
        case 4:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_THIRD_4 ? false : true);
            break;
    }

    print '<td align="center">';
    if (!empty($conf->use_javascript_ajax)) {
        print ajax_constantonoff('MAIN_MODULE_LABELPRINT_THIRD_' . $i);
    } else {
...
while ($i < $nbofloop) {
    ...
    switch ($i) {
        case 0:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_LABELS_THIRD_0 ? false : true);
            break;
        case 1:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_LABELS_THIRD_1 ? false : true);
            break;
        case 2:
            $verif = (!$conf->global->MAIN_MODULE_LABELPRINT_LABELS_THIRD_2 ? false : true);
            break;
        /*case 3:
...
print '<td align="center" width="60">' . $html->selectyesno("labA4", $conf->global->LABELPRINT_THIRD_A4, 1) . '</td>';
print '</tr></table>';
/*
 *  General Optiones
*/
...
print '<textarea name="labFreeText" class="flat" cols="120">' . $conf->global->LAB_THIRD_FREE_TEXT . '</textarea>';
print '</td></tr>';
