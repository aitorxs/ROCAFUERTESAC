...
if ($id > 0 || !empty($ref))
{
	...
	if ($object->fetch($id, $ref) > 0)
	{
		...
		dol_fiche_head($head, 'labelprint', $langs->trans('InvoiceCustomer'), 0, 'bill');
		...
		//{
			...
			if ($user->rights->societe->creer)
			{
				...
				if($object->statut>0) print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$id.'&amp;action=add">'.$langs->trans("AddToPrint").'</a>';
