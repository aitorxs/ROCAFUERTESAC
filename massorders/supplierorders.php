...
$sref_supplier=GETPOST('sref_supplier','alpha');
...
if ($action == 'confirm_massinvoicing' && $confirm == 'yes') {
    ...
    while ($i < $nume) {
        if (empty($paymode[$i])) {
            ...
            if (empty($static_societe->mode_reglement_supplier_id)) {
                $paymode[$i] = $conf->global->MASSO_PAY_MODE;
            } else {
                $paymode[$i] = $static_societe->mode_reglement_supplier_id;
            }
        }
        $i++;
    }
    ...
    $result = restrictedArea($user, 'fournisseur', $id, '', '', 'facture', $fieldid);
    ...
    foreach ((array)$array as $comp_id => $value) {
        if ($groupmonth && $grouppaymode) {
            foreach ($value as $date_id => $value1) {
                foreach ($value1 as $pay_id => $value2) {
                    ...
                    $result = $massor->invoicing_supplier($comp_id, $validateInvoice, $date_id, $pay_id);
                    if (!$result) {
                        $error++;
                    }
                }
            ...
            if ($groupmonth) {
                foreach ($value as $date_id => $value1) {
                    ...
                    $result = $massor->invoicing_supplier($comp_id, $validateInvoice, $date_id);
                    if (!$result) {
                        $error++;
                    }
                }
                ...
                if ($grouppaymode) {
                    foreach ($value as $pay_id => $value1) {
                        $massor->orders = $value1;
                        $result = $massor->invoicing_supplier($comp_id, $validateInvoice, '', $pay_id);
                        if (!$result) {
                            $error++;
                        }
                    }
                    ...
                    $massor->orders = $value;
                    $facid = $massor->invoicing_supplier($comp_id, $validateInvoice);
                    if (!$facid) {
                        $error++;
                    }
                }
        ...
        if (($conf->global->MASSO_AUTO_PDF || $conf->global->MASSO_AUTO_MAIL) && $facid) {
            ...
            if ($conf->global->MAIN_MULTILANGS && empty($newlang)) {
                $newlang = $fac->thirdparty->default_lang;
            }
            if (!empty($newlang)) {
        ...
        if ($conf->global->MASSO_AUTO_MAIL && $result) {
            ...
            $result = $massor->search_mail(142);
            if ($result < 0) {
                $error++;
            } else {
                if ($fac->statut > 0) {
                    $result = $massor->send_mail($fac, 2);
                }
                if ($result < 0) {
                    $error++;
                }
...
llxHeader('',$langs->trans('SuppliersOrders'),$helpurl);
...
$company=$form->select_company($company_start,'company_start','s.fournisseur=1',1).' - '.$form->select_company($company_end,'company_end','s.fournisseur=1',1);
$category=$form->select_all_categories(1,$category_start,'category_start').' - '.$form->select_all_categories(1,$category_end,'category_end');
...
// Company Range
print '<tr>';
print '<td>'.$langs->trans('SupplierRange').'</td>';
print '<td colspan="3">';
print $company;
print '</td>';
print '</tr>';
...
if($conf->categorie->enabled) {
    print '<tr>';
    print '<td>' . $langs->trans('SupplierCategoryRange') . '</td>';
    print '<td colspan="3">';
    print $category;
    print '</td>';
    print '</tr>';
...
if ($user->rights->fournisseur->lire || $socid) {
...
if (!empty($company_start) || !empty($company_end)) {
    ...
    $sql = 'SELECT rowid FROM ' . MAIN_DB_PREFIX . "societe WHERE nom >= '" . $company_start_nom . "' AND nom <= '" . $company_end_nom . "' AND fournisseur=1";
    $result = $db->query($sql);
    if ($result) {
...
if (!empty($category_start) || !empty($category_end)) {
    ...
    $sql = 'SELECT rowid FROM ' . MAIN_DB_PREFIX . "categorie WHERE label >= '" . $category_start_lab . "' AND label <= '" . $category_end_lab . "' AND type=1";
    $result = $db->query($sql);
    if ($result) {
        $num = $db->num_rows($result);
        $i = 0;
...
$sql = 'SELECT s.nom, s.rowid as socid, s.fournisseur, c.rowid, c.ref, c.total_ht, c.ref_supplier,';
$sql.= ' c.date_valid, c.date_commande, c.date_livraison, c.fk_statut, c.fk_mode_reglement';
...
if (!empty($categories)) {
    ...
    if (version_compare(DOL_VERSION, 3.8) >= 0) {
        $sql .= ' left join ' . MAIN_DB_PREFIX . 'categorie_fournisseur as cat on s.rowid = cat.fk_soc ';
    } else {
        $sql .= ' left join ' . MAIN_DB_PREFIX . 'categorie_fournisseur as cat on s.rowid = cat.fk_societe ';
    }
}

// We'll need this table joined to the select in order to filter by sale
if ($search_sale > 0 || (! $user->rights->fournisseur->lire && ! $socid)) {
...
if (!empty($sref_supplier)) {
    $sql .= ' AND c.ref_supplier LIKE \'%' . $db->escape($sref_supplier) . '%\'';
}

$sql.= ' AND c.fk_statut IN (3,4,5)';
$sql.= ' AND c.rowid NOT IN (SELECT e.fk_source FROM '.MAIN_DB_PREFIX.'element_element e';
$sql.= " WHERE c.rowid = e.fk_source AND e.sourcetype = 'order_supplier' AND e.targettype = 'invoice_supplier')";
...
if ($search_user > 0)
{
    $sql.= " AND ec.fk_c_type_contact = tc.rowid AND tc.element='order_supplier' AND tc.source='internal' AND ec.element_id = c.rowid AND ec.fk_socpeople = ".$search_user;
}
...
if ($resql) {
    ...
    if ($sref_supplier) {
        $param .= '&sref_supplier=' . $sref_supplier;
    }
    if ($search_user > 0) {
    ...
    print_liste_field_titre($langs->trans('Company'), $_SERVER['PHP_SELF'], 's.nom', '', $param, '', $sortfield,
        $sortorder);
    print_liste_field_titre($langs->trans('RefSupplierOrder'), $_SERVER['PHP_SELF'], 'c.ref_supplier', '', $param, '',
        $sortfield, $sortorder);
    print_liste_field_titre($langs->trans('OrderDate'), $_SERVER['PHP_SELF'], 'c.date_commande', '', $param,
        'align="right"', $sortfield, $sortorder);
    print_liste_field_titre($langs->trans('DeliveryDate'), $_SERVER['PHP_SELF'], 'c.date_livraison', '', $param,
        'align="right"', $sortfield, $sortorder);
    ...
    print '<input class="flat" type="text" size="10" name="sref_supplier" value="' . $sref_supplier . '">';
    ...
    while ($i < $num) {
        ...
        if (($orders_list[$i]->fk_statut > 0) && ($orders_list[$i]->fk_statut < 5) && $db->jdate($orders_list[$i]->date_valid) < ($now - $conf->commande->fournisseur->warning_delay)) {
        ...
        $filedir = $conf->fournisseur->commande->dir_output . '/' . dol_sanitizeFileName($orders_list[$i]->ref);
        ...
        $companystatic->fournisseur = $orders_list[$i]->fournisseur;
        print '<td>';
        ...
        print '<td>' . $orders_list[$i]->ref_supplier . '</td>';
