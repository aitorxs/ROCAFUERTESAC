...
if ($action === 'confirm_massinvoicing' && $confirm === 'yes')
{
	...
	unset($_SESSION['orders_selected'], $_SESSION['orders'], $_SESSION['companies'], $_SESSION['paymode'],
		$_SESSION['ref'], $_SESSION['order_date']);
        ...
        if ($groupmonth && $grouppaymode) {
            foreach ($value as $date_id => $value1) {
                foreach ($value1 as $pay_id => $value2) {
                    ...
                    $result = $massor->invoicing_propals($comp_id, $closeOrders, $validateInvoice, $date_id, $pay_id);
                    if (!$result) {
                        $error++;
                    }
                }
            ...
            if ($groupmonth) {
                foreach ($value as $date_id => $value1) {
                    ...
                    $result = $massor->invoicing_propals($comp_id, $closeOrders, $validateInvoice, $date_id);
                    if (!$result) {
                        $error++;
                    }
                }
                ...
                if ($grouppaymode) {
                    foreach ($value as $pay_id => $value1) {
                        $massor->orders = $value1;
                        $result = $massor->invoicing_propals($comp_id, $closeOrders, $validateInvoice, '', $pay_id);
                        if (!$result) {
                            $error++;
                        }
                    }
                    ...
                    $massor->orders = $value;
                    $facid = $massor->invoicing_propals($comp_id, $closeOrders, $validateInvoice);
                    if (!$facid) {
                        $error++;
                    }
                }
...
llxHeader('',$langs->trans('Proposals'),$helpurl);
...
// Ligne de titre
print '<tr>';
print '<td valign="top" colspan="4">'.$langs->trans('PropalsFilter').'</td>';
print '</tr>';

// Date Range
print '<tr>';
print '<td>'.$langs->trans('PropalDate').'</td>';
print '<td colspan="3">';
print $period;
print '</td>';
print '</tr>';
...
$sql.= ' c.date_valid, c.datep, c.fin_validite, c.fk_statut, c.fk_mode_reglement';
...
$sql.= " WHERE c.rowid = e.fk_source AND e.sourcetype = 'propal' AND e.targettype = 'facture')";*/
	
$sql.= " AND c.datep BETWEEN '".$db->idate($date_start)."' AND '".$db->idate($date_end)."'";
...
if ($search_user > 0)
{
    $sql.= " AND ec.fk_c_type_contact = tc.rowid AND tc.element='propal' AND tc.source='internal' AND ec.element_id = c.rowid AND ec.fk_socpeople = ".$search_user;
}
...
if ($resql)
{
	...
	if ($socid)
	{
		$soc = new Societe($db);
		$soc->fetch($socid);
		$title = $langs->trans('ListOfPropals') . ' - '.$soc->name;
	}
	else
	{
		$title = $langs->trans('ListOfPropals');
	}
	if ($viewstatut == -3) {
        $title .= $langs->trans('PropalsToBill');
    }
	...
	print_liste_field_titre($langs->trans('RefCustomer'),$_SERVER['PHP_SELF'],'c.ref_client','',$param,'',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans('PropalDate'),$_SERVER['PHP_SELF'],'c.date_commande','',$param, 'align="right"',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans('DateEndPropalShort'),$_SERVER['PHP_SELF'],'c.date_livraison','',$param, 'align="right"',$sortfield,$sortorder);
	...
	while ($i < $num)
	{
		...
		if (($orders_list[$i]->fk_statut = 2) && $db->jdate($orders_list[$i]->date_valid) < ($now - $conf->propal->facturation->warning_delay)) {
		...
		$filedir=$conf->propal->dir_output . '/' . dol_sanitizeFileName($orders_list[$i]->ref);
		...
		// Order date
		$y = dol_print_date($db->jdate($orders_list[$i]->datep),'%Y');
		//$m = dol_print_date($db->jdate($orders_list[$i]->date_commande),'%m');
		$ml = dol_print_date($db->jdate($orders_list[$i]->datep),'%B');
		$d = dol_print_date($db->jdate($orders_list[$i]->datep),'%d');
		print '<td align="right">';
		print $d.' '.$ml.' '.$y;
		print '</td>';

		// Delivery date
		$y = dol_print_date($db->jdate($orders_list[$i]->fin_validite),'%Y');
		//$m = dol_print_date($db->jdate($orders_list[$i]->date_livraison),'%m');
		$ml = dol_print_date($db->jdate($orders_list[$i]->fin_validite),'%B');
		$d = dol_print_date($db->jdate($orders_list[$i]->fin_validite),'%d');
		print '<td align="right">';
		print $d.' '.$ml.' '.$y;
		print '</td>';

		// Statut
		print '<td align="right" nowrap="nowrap">'.$generic_commande->LibStatut($orders_list[$i]->fk_statut,5).'</td>';
	...
	if (isset($_POST['buttonInvoice']) && ! $_REQUEST['cancel']) {
		...
		if($user->rights->commande->cloturer){
			$formquestionmassinvoicing[]=array('type' => 'checkbox', 'name' => 'autocloseorders','label' => $langs->trans('CloseProcessedPropalsAutomatically'), 'value' => 1, 'size'=>5);
		}
        ...
        } else 	if($user->rights->facture->valider && $conf->global->STOCK_CALCULATE_ON_BILL == 0){
			$formquestionmassinvoicing[]=array('type' => 'checkbox', 'name' => 'validateinvoice','label' => $langs->trans('ValidateInvoicesAutomatically'), 'value' => 1, 'size'=>5);
		}
		
		print $form->formconfirm($_SERVER['PHP_SELF'],$langs->trans('MassInvoicing'),$langs->trans('ConfirmMassInvoicing'),'confirm_massinvoicing',$formquestionmassinvoicing,'yes',1,250,420);
	}
