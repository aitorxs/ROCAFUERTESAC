...
if ($resql) {
    ...
    print_liste_field_titre($langs->trans('Company'), $_SERVER['PHP_SELF'], 's.nom', '', $param, '', $sortfield,
        $sortorder);
    print_liste_field_titre($langs->trans('RefCustomerOrder'), $_SERVER['PHP_SELF'], 'c.ref_client', '', $param, '',
        $sortfield, $sortorder);
    ...
    while ($i < $num) {
        ...
        if (($orders_list[$i]->fk_statut > 0) && ($orders_list[$i]->fk_statut < 3) && $db->jdate($orders_list[$i]->date_valid) < ($now - $conf->commande->client->warning_delay)) {
            print img_picto($langs->trans('Late'),
                'warning');
        }
        print '</td>';
        ...
        if (!empty($conf->facture->enabled) && $user->rights->facture->creer) {
            ...
            if (($orders_list[$i]->fk_statut > 0 && $orders_list[$i]->fk_statut < 3) || ($orders_list[$i]->fk_statut == 3 && $orders_list[$i]->facturee == 0)) {
                ...
                print img_picto($langs->trans('CreateInvoiceForThisCustomer') . ' : ' . $companystatic->name,
                        'object_bill') . '</a>';
            }
        }
        print '</td>';
    ...
    if (isset($_POST['buttonInvoice']) && !$_REQUEST['cancel']) {
        ...
        if ($user->rights->commande->cloturer) {
            $formquestionmassinvoicing[] = array(
                ...
                'label' => $langs->trans('CloseProcessedOrdersAutomatically'),
                'value' => 1,
                'size' => 5
            );
        }
