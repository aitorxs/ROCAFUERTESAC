...
var TPV = jQuery.Class({
    ...
    getDraft: function () {
        ...
        var saleref = "$('#placeName').html('newTicket');";
        list += '<li data-icon="false" data-theme="e"><a onclick="_TPV.ticket.newTicket();' + saleref + '" href="#">';
        ...
        $.each(result, function (i, item) {
            saleref = "$('#placeName').html('" + item['ticketnumber'] + "');";
            list += '<li data-icon="false" ><a onclick="_TPV.getTicket(' + item['id'] + ',true);' + saleref + '" href="#">';

            //list += '<li><a onclick="_TPV.ticket.getTicket('+item.rowid+')" href="#">';
            list += item['ticketnumber'];
            list += '</a></li>';
        });
        $.mobile.changePage("#services", {transition: "slideup"});
