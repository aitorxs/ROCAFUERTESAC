...
/**
 *		\file        /pos/backend/terminal/annuel.php
 *		\ingroup     pos
...
//$sql.= " AND (type = 0)";
//$sql.= " AND total_ht >= 0";
if ($acct->id) $sql .= " AND fk_cash IN (".$acct->id.")";
$sql.= " GROUP BY dm";

$sql .= " UNION SELECT sum(fac.total_ttc) as amount, date_format(fac.datec,'%Y-%m') as dm";
...
//$sql .= " AND fac.total >=0";
if ($acct->id) $sql .= " AND pf.fk_cash IN (".$acct->id.")";
$sql .= " GROUP BY dm";
$sql .= " ORDER BY dm";
...
dol_fiche_head($head,'annual',$langs->trans("Cash"),0,'barcode');
