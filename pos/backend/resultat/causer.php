...
$sql = "SELECT s.rowid as socid, s.lastname, s.firstname, sum(f.total_ht) as amount, sum(f.total_ttc) as amount_ttc";
...
//$sql.= ")";
$sql.= " AND f.fk_user_close = s.rowid";
...
$sql.= " GROUP BY s.rowid, s.lastname";
...
if ($result)
{
	...
	while ($i < $num)
	{
		...
		$name[$obj->socid] = $obj->firstname.' '.$obj->lastname;
...
if (count($amount))
{
	...
	foreach($arrayforsort as $key=>$value)
	{
		...
		$url = dol_buildpath('/pos/backend/liste.php?userid='.$key.'',1);
