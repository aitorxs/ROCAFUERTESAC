...
if (! $res) $res=@include("../../../main.inc.php");                // For "custom" directoryrequire_once DOL_DOCUMENT_ROOT.'/core/class/html.formfile.class.php';
...
$search_user_seller = GETPOST('search_user_seller','alpha');
$search_terminal = GETPOST('search_terminal','alpha');
$day	= GETPOST('day','int');
...
$arrayfields=array(
    ...
    'f.ref_client'=>array('label'=>$langs->trans("RefCustomer"), 'checked'=>0),
    ...
    'ca.name'=>array('label'=>$langs->trans("Cash"),'checked'=>1),
    ...
    'dynamount_payed'=>array('label'=>$langs->trans("Received"), 'checked'=>1),
...
$sql = 'SELECT';
if ($search_product_category > 0) $sql = 'SELECT DISTINCT';
...
$sql.= ' ca.name as cash, ca.rowid as cashid, f.fk_user_valid as uservalid';
...
if ($search_product_category > 0) $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'facturedet as pd ON f.rowid=pd.fk_facture';
if ($search_product_category > 0) $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'categorie_product as cp ON cp.fk_product=pd.fk_product';
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."user as u ON u.rowid = f.fk_user_valid";
...
if (GETPOST('search_terminal'))
{
    $sql.= ' AND ca.name LIKE \'%'.$db->escape(trim(GETPOST('search_terminal'))).'%\'';
}
...
if ($search_user_seller) $sql .= natural_search(array('u.lastname','u.firstname'), $search_user_seller);
if ($search_terminal) $sql .= natural_search('ca.name', $search_terminal);
...
{
    if ($viewstatut == '0') $sql.=" AND f.fk_statut = 0";  // draft
    if ($viewstatut == '1') $sql.=" AND f.fk_statut = 1";  // unpayed
    if ($viewstatut == '2') $sql.=" AND f.fk_statut = 2";  // payed     Not that some corrupted data may contains f.fk_statut = 1 AND f.paye = 1 (it means payed too but should not happend. If yes, reopen and reclassify billed)
    if ($viewstatut == '3') $sql.=" AND f.fk_statut = 3";  // abandonned
}
...
if ($resql)
{
	...
	if ($search_user_seller) $param.='&search_user_seller='.$search_user_seller;
	if ($search_terminal)    $param.='&search_terminal='.$search_terminal;
    ...
    if (! empty($arrayfields['ca.name']['checked']))              print_liste_field_titre($arrayfields['ca.name']['label'],$_SERVER['PHP_SELF'],'','',$param,'',$sortfield,$sortorder);
    ...
    //Terminal
    if (! empty($arrayfields['ca.name']['checked'])) {
        print '<td class="liste_titre" align="left"><input class="flat" type="text" name="search_terminal" value="'.$search_terminal.'"></td>';
    }

    //User
    if (! empty($arrayfields['u.lastname']['checked'])) {
        print '<td class="liste_titre" align="left"><input class="flat" type="text" name="search_user_seller" value="' . $search_user_seller . '"></td>';
    }
	// Thirpdarty
        ...
        while ($i < min($num,$limit))
        {
            ...
            if (! empty($arrayfields['u.lastname']['checked'])) {
                ...
                if ($obj->uservalid>0)
                {
                    ...
                    $userstatic->fetch($obj->uservalid);
